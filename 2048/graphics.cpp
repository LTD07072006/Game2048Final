...
void render(int score, int highscore, TTF_Font* font)
{
    ...
    // Tính toán kích thước và vị trí của khung bao quanh các ô
    SDL_Rect boardBorder;
    boardBorder.x = offsetX + TILE_MARGIN;  // Vị trí X của khung (căn theo offset bảng)
    boardBorder.y = offsetY + TILE_MARGIN;  // Vị trí Y của khung (căn theo offset bảng)
    boardBorder.w = COLUMNS * TILE_SIZE_DYNAMIC + (COLUMNS - 1) * TILE_MARGIN;  // Chiều rộng khung
    boardBorder.h = ROWS * TILE_SIZE_DYNAMIC + (ROWS - 1) * TILE_MARGIN;        // Chiều cao khung

    SDL_SetRenderDrawColor(renderer, FONT_COLOR.r, FONT_COLOR.g, FONT_COLOR.b, FONT_COLOR.a);
